...
| scope_bounded | Issue body, the opener's author_association, labels, and comment thread | Pass unless one of these is true: the issue is itself an umbrella or tracking issue (it says so, is labeled tracking/umbrella/meta/roadmap, or is a checklist of separate deliverables meant to be split into separate PRs); the thread shows the design is still debated with no maintainer decision; a maintainer says the fix touches core internals; the issue is a pure usage question; it has been open for years with several closed, unmerged PR attempts; or it was opened by a `[bot]` account and has no labels and no comment from an Owner, Member, or Collaborator. A single bug report that lists candidate causes or suggested fixes is one bounded issue, not an umbrella. A short body or missing reproduction steps is not a failure. | required |
